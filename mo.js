...
function moZoom(mo, zm, pt, tr) {
	if(mo === undefined) {return}
	let sc = -1;
	if(zm === undefined) {zm = mo.scale; pt = mo.center; tr = true}
	else if(pt === undefined || !Array.isArray(pt)) {pt = mo.center; tr = true}
...
/*function moZoom(mo, x, y) {
	if(mo === undefined) {return}
	let z = -1;
	if(x === undefined) {x = mo.scale; y = true}
	if(y === undefined) {y = false}
	// slider
	if(!isNaN(x)) {
		x = Number(x);
		z = ((mo.scales[mo.scales.length - 1] - mo.scales[0]) * x) + mo.scales[0]
	}
	// scales
	else if(x != undefined) {
		let prev = mo.scales[0], next = mo.scales[mo.scales.length - 1];
		mo.scales.every((sc, i) => {
			if(sc < mo.scale) {return true}
			else if(sc > mo.scale) {
				if(i - 1 > 0) {prev = mo.scales[i - 1]}
				if(i < mo.scales.length - 1) {next = mo.scales[i]}
				return false
	...
	if(z < mo.scales[0]) {z = mo.scales[0]}
	else if(z > mo.scales[mo.scales.length - 1]) {z = mo.scales[mo.scales.length - 1]}
	mo.cont.style.transform = "scale(" + z + "," + z + ")"; mo.scale = z;
	if(y) {moSnap(mo)}
	if(y) {mo.controls.sliders.forEach(sl => {
		sl.value = (z - mo.scales[0]) / (mo.scales[mo.scales.length - 1] - mo.scales[0])})}
...
moRef.forEach(mo => {
	...
	if(mo.mo.querySelector("[data-mo='-']")) {
		...
		mo.controls.minus.forEach(x => {x.addEventListener("click", () => {moZoom(mo, "-")})})
	}
	if(mo.mo.querySelector("[data-mo='+']")) {
		mo.controls.plus = moToArray(mo.mo.querySelectorAll("[data-mo='+']"));
		mo.controls.plus.forEach(x => {x.addEventListener("click", () => {moZoom(mo, "+")})})
	}
	if(mo.mo.querySelector("[data-mo='slider']")) {
		mo.controls.sliders = moToArray(mo.mo.querySelectorAll("[data-mo='slider']"));
		mo.controls.sliders.forEach(x => {x.addEventListener("input", () => {moZoom(mo, x.value)})})
	}
	// keys
